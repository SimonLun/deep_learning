...
class MysqlClient(object):
	...
	def __init__(self, username=None, password=None, database_name=None, MYSQL_DATABASE=None):
		self.username = username if username is not None else MYSQL_USER
		self.password = password if password is not None else MYSQL_PASSWORD
		self.database_name = database_name if database_name is not None else MYSQL_DATABASE
	...
	def __get_conn(self):
		...
		try:
			conn = pymysql.connect(**default_connect_params)
			return conn
