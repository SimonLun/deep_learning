...
class WindSqlEngine(object):
    ...
    def get_active_stocks_df(self, startdate = None, enddate=None,flag = None) -> pd.DataFrame:
        ...
        columns_all = ['ticker','All', 'All_ex_BJ', 'Investable_ex_BJ', 'Investable_ex_BJ_ipo60', 'Investable_ex_BJ_ipo90',
                       'Float2000']
    ...
    def get_st_stocks_status(self, start_date=None, end_date=None):
        ...
        for i in stat_df.index:
            ...
            if stat_df.loc[i].REMOVE_DT == 21000101:
                ...
                date = [dt for dt in date_all if (dt >= max(stat_df.loc[i].ENTRY_DT, int(start_date))) and (dt < stat_df.loc[i].REMOVE_DT)]
    ...
    def get_Float2000_market_cap(self, start_date=None, end_date=None):
        ...
        market_cap_sql = read_sql_file("sql_wind_market_cap").format(new_start_date, end_date)
    ...
    def get_industry_code(self) -> pd.DataFrame:
        ...
        for dt in self.busy_days:
            wind_industrycode_sql = read_sql_file("sql_wind_industrycode").format(str(dt))
        ...
        try:
            df0.loc[:, 'IndustryCode'] = replace(df0['IndustryCode'], para['industry_index_1'],
                                                 para['industry_index_1_hash'])
        except Exception as err:
            logger.error(err)
        df0.loc[:, 'Date'] = df0['Date'].astype(str)
        df0 = df0.set_index(['Date', 'Ticker'])
        return df0
    ...
    def get_consensusexpectation_factor(self,start_date=None,end_date=None):
        ...
        consensusexpectation_factors = self.mysql_db_conn.read_sql(consensusexpectation_factor_sql.format(start_date, end_date))
